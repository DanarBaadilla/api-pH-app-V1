...
# FCN PSP Net model class for semantic segmentation
##############################################This is a standart FCN with the last layer split into prediction of binary map for every class########################################################################333
class Net(nn.Module):
######################################Build FCN net layer##################################################################################
    def __init__(self, CatDict):
        ...
        # --------------Build layers for standart FCN with only image as input------------------------------------------------------
            ...
            for f,nm in enumerate(CatDict):
                    ...
                    self.OutLayersDict[nm]= nn.Conv2d(256, 2, stride=1, kernel_size=3, padding=1, bias=False) # Fine tune untuk Binary Mask (Output = 1) : self.OutLayersDict[nm]= nn.Conv2d(256, 2, stride=1, kernel_size=3, padding=1, bias=False)
...
#------------------Run psp  Layers----------------------------------------------------------------------------------------------
                ...
                for i,PSPLayer in enumerate(self.PSPLayers): # run PSP layers scale features map to various of sizes apply convolution and concat the results
                      NewSize=(np.array(PSPSize)*self.PSPScales[i]).astype(np.int64) #np.int
